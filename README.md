...
docs/
  ...
  quality-assessment.md                 the QA scores as three figures, with notes on how to read them
  figures/                              PNG and SVG, regenerated by tools/render_qa_figures.py
tools/
  render_qa_figures.py                  rebuilds docs/figures/ from the QA sheets
search/
...
**CSV does not render in a browser, so the quality assessment is also drawn.**
[`docs/quality-assessment.md`](docs/quality-assessment.md) shows all 34 studies against
all ten items, which criteria the corpus meets, and the distribution of total scores.
The figures are generated from `data/03_quality_assessment_scores.csv` and change nothing
about it — the CSV stays canonical.
