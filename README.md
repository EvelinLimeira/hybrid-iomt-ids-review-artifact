...
|---|---|---|---|
| Explainability mechanisms in 10 studies (29.4%) | `07 Explainability`; `08 XAI Present` | 10 studies with a named mechanism; `XAI Present` = Yes in 8, Partially in 1 | Agree. The paper previously reported 15; that figure could not be reproduced from the sheets or from the full texts and was corrected. Three further candidates were checked and rejected: `R0531` defers explainability to future work, `R0639`'s attention is an internal aggregation step, and `R1520`'s attention fuses streams rather than explaining them. `R0696` reports a SHAP column in its results but is `not reported` in `07`; `R0175` is `Feature importance` in `07` describing Random Forest's role rather than a reported mechanism. The two offset. |
| Threshold strategy reported in 13 studies (38.2%); unreported where applicable in 21 | `07 Threshold Strategy` | 11 populated, 23 `not reported` | The paper counts two procedures evidenced in the full text but left blank in the sheet: `R0753`'s triple hierarchical energy and correlation gate, calibrated on clean and noisy reference signals, and `R0639`'s dual confidence bands with device-criticality weighting. Both are now populated in `07`. The paper previously reported 15; the two remaining studies could not be found. |
| Device-specific validation absent (0 studies) | `08 Per-device Eval.`; `QA9` | `Yes` in 3 (`R0696`, `R0736`, `R0791`), matching `QA9` = Yes | Different constructs. The sheet column and `QA9` record per-device **profiling or modelling**: `R0696` computes a per-device trust score, `R0736` and `R0791` report energy and latency per node. None reports detection performance broken down by device against a pooled baseline, which is what Fig. 4's bar counts. The bar is labelled *Device-specific validation* to keep the two apart. |
| Code available in 2 studies (5.9%) | `08 Code Available` | `Yes` 1 | `R1516` releases code at `github.com/syriuslab/fusion_framework` in its data-availability statement but was recorded as `not reported`; the sheet is now corrected to `Yes`, so both sources agree on 2. The other is `R0571` (`github.com/anurekha1990/IOMT`). |
| Component or branch ablation in 11 studies (32.4%) | `08 Component Ablation` | `Yes` 11 | This column did not exist when the paper was written and the bar had no traceable source. It was coded afterwards from the full texts: 13 studies use the word *ablation*, of which `R0791` describes prior work and `R0419` ablates preprocessing steps rather than model components. The remaining 11 reproduce the published figure. |
